...
pipeline = Pipeline(steps=[
    ...
    ('SelectFromModel_ElasticNet', SelectFromModel(ElasticNet())),
    ('LinearRegression', LinearRegression()),
])
...
pipeline.get_params('LinearRegression')
...
def regression_results(y_true, y_pred):
    ...
    # TXT Report
    filename_scores = open(os.path.join(file_dir, 'LinearRegression_trained_pipeline_scores.txt'), "w",
                           encoding='utf-8')
    filename_scores.write("Problem: Regression" + "\n")
    filename_scores.write("model's name: " 'LinearRegression' + "\n")
    ...
    filename_scores_html = open(os.path.join(file_dir, 'LinearRegression_trained_pipeline_scores.html'), "w",
                                encoding='utf-8')
...
filename = os.path.join(file_dir, 'LinearRegression_trained_pipeline.joblib')
...
fs = pipeline.named_steps['SelectFromModel_ElasticNet']
support_mask = fs.get_support()
fs_names = np.array(all_features)
# Assuming the preprocessor step in your pipeline does all transformations
feature_names_transformed = pipeline.named_steps['preprocessor'].get_feature_names_out()
selected_features = [feature for feature, is_selected in zip(feature_names_transformed, support_mask) if is_selected]

# selected_features = fs_names[fs.get_support()]

sel_fea = []
for feature in selected_features:
    sel_fea.append(feature.split('__')[1])
print(sel_fea)
selected_features_file = open(os.path.join(file_dir, 'selected_features'), "wb")
dill.dump(sel_fea, selected_features_file)
...
shap.initjs()
shap_explainer = shap.LinearExplainer(pipeline.named_steps['LinearRegression'], masker=shap_data)
...
try:
    ...
    permuter = PermutationImportance(pipeline.named_steps['LinearRegression'],
                                     scoring='r2', cv='prefit', n_iter=2, random_state=0)
    permuter.fit(eli5_data, y_test)
    # explain the predictions of the model using permutation importance
    if len(sel_fea) != 0:
        explanation = eli5.explain_weights(permuter, top=None, feature_names=sel_fea)
    else:
